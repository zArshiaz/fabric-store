...
export default function AddressModal({type, show, setShow, data}: addressModalProps) {
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        if (type === 'add') {
            ...
            await editAddress(data!._id, payload)
                .then(data => {
                    reset();
                    setShow(false);
                })
    ...
    return (
        <div onClick={() => setShow(false)}
             className={`${show ? 'flex' : 'hidden'} fixed inset-0  m-auto bg-black/50 z-50 justify-center items-center`}>
            <form onClick={(e: React.MouseEvent<HTMLFormElement>) => e.stopPropagation()}
                  onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white p-5 rounded-xl ">
