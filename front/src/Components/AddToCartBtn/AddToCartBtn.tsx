...
export default function AddToCartBtn({className,setCountAction,count,min=0.5 ,max}: {className?: string,count:number,min?:number,max:number,setCountAction:React.Dispatch<React.SetStateAction<number>>}) {
    ...
    const inc=(step:number)=>{
        ...
        if(count+step>max){
            toast.fire({
                icon:"info",
                text:'کل محصول در سبد خرید شما قرار دارد.'
            })
            setCountAction(max)
            return
        }
        setCountAction(prev => {
            const nexValue=Math.min(max,prev+step)
            return Math.round(nexValue*10)/10;
        })
    }
    const dis=(step:number)=>{
        setCountAction(prev => {
            const nexValue=Math.max(min,prev-step)
            return Math.round(nexValue*10)/10;
        })
    }
