                ...
                return (
                    <div key={step.id} className={'flex items-center'}>
                        ...
                        {index < steps.length - 1 && (
                            <div key={step.path}
                                className={`flex-1 h-[1px] w-10 sm:w-28 md:w-36 border border-dashed mx-2 transition-all duration-300 ${index < activeIndex ? 'border-red-500 border-solid' : 'border-gray-400'}`}
                            />
                        )}
                    </div>
