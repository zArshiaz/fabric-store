...
const SidebarMenu=memo(  function ({sidebarList, show, setShow}: {
    ...
    return (
        <>
            <div
                className={`fixed z-50 inset-y-0 flex flex-col  w-64 bg-white  border-l border-red-700 overflow-y-auto transition-[right] duration-300 ease-in-out ${show ? 'right-0' : '-right-64'}`}>
                {/* header */}
                ...
                {/* body */}
                <div className="flex-1 flex flex-col text-zinc-900 px-3 py-4 gap-1">
                    {sidebarList.map((item: TNav, idx: number) => {
                        const isActive = pathname === item.href
                        const isOpen = openIdx === idx
                        return (
                            <div key={item.href}>
                                <div className={`flex items-center justify-between rounded-lg ${isActive ? 'text-red-600 bg-red-300/30 shadow shadow-red-200' : ''}`}>
