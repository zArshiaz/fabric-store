...
import {checkEmailApi, editUserApi} from "@/api/auth";
...
function ProfilePage() {
    ...
    const submit =async (d:TForm) => {
      //    await fetch(`https://nakhshin.shop/api/auth/${userInfo?.id}`,{
      //     method: "PUT",
      //     headers: {
      ...
      // })
        await editUserApi(d,userInfo!.id)
          .then(data => {
              Swal.fire({
                    ...
                    <input type="email"
                           {...register('email',{
                               ...
                               validate:async (v)=>{
                                  ...
                                  const data= await checkEmailApi({email:v})
                                   if(data.exist) return 'ایمیل تکرای'
                                   return true
                               }
                           })}
                           dir={'ltr'}
