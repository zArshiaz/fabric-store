...
import {checkEmailApi, registerApi} from "@/api/auth";
...
function Register() {
    ...
    const onSubmit = async (values: TFormValues) => {
        ...
        try {
            setLoading(true);
            const data = await registerApi(newValues);
            login(data.user);
            ...
            <form onSubmit={handleSubmit(onSubmit)} className={'space-y-6'} noValidate={true}>
                    ...
                    <input type="email" dir={'ltr'}
                           {...register('email', {
                               ...
                               validate: async (v) => {
                                   ...
                                   try {
                                       const data = await checkEmailApi(e);
                                       if (data.exist) {
                ...
                <div className={'relative'}>
                    <input type="text" dir={'ltr'}
                           {...register('phone', {
                               required: "تلفن الزامی",
                               pattern: {value: Regex.phoneNumber, message: "تلقن نامعتبر"},
                           })}
                           className=" w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-200 focus:border-transparent outline-none transition-all duration-300"
